...
**Перед записом будь-якого `.side` файлу — перевірити чи він вже існує.**
Якщо існує і містить `"locked": true` — **не перезаписувати**. Файл відредагований вручну. Пропустити і повідомити у звіті.

Нові файли створювати **без** поля `locked` (або з `"locked": false`) — щоб не блокувати автоматичне оновлення до першої ручної правки.
...
| Контейнер модалки | `[data-modal-ready="true"]` |
...
> **Важливо — поле `"url"`:** воно обов'язково має бути порожнім: `"url": ""`. Katalon Recorder використовує "Base URL" зі своїх внутрішніх налаштувань. Жорстке прописування домену (напр. `http://localhost`) зробить скрипт непрацездатним на інших оточеннях або при зміні порта.
